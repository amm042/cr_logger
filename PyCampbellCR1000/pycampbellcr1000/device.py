...
from PyLink.pylink import link_from_url
...
class CR1000(object):
    ...
    def __init__(self, link, dest_addr=0x001, dest_node=None, src_addr=0x802,
        ...
        # flush the input faster than wait_packet
        link.read(16*1024, timeout=0.001)        
        ...
        for i in xrange(3):
            try:
                if self.ping_node():
                    ...
                    # setting id 1 is the serial number on the CR6
                    hdr, msg, send_time = self.send_wait(self.pakbus.get_getsettings_cmd(1, 2))
                    
                    if 'Settings' in msg and msg['Settings'] != []:
                        self.serialNo = msg['Settings'][0]['DecodeValue']
                    else:
                        self.serialNo = None
                        
                    LOGGER.info("Serial number is {}".format(self.serialNo))                                 
    ...
    def wait_for_message(self, timeout = 10):        
        '''Wait for a pakbus message, usefull to receive one-way messages'''
        begin = time.time()
        while time.time() - begin < timeout:            
            h, m = self.pakbus.wait_packet()
    ...
    def getfile(self, filename):
        ...
        LOGGER.info('Try get file {}'.format(filename))
        self.ping_node()
        data = []
    ...
    def _collect_data(self, tablename, start_date=None, stop_date=None):
        ...
        # Get table number
        tablenbr = None
        #decode ascii now makes utf-8 by default so don't convert to bytes or
