...
class PakBus(object):
    ...
    MSGTYPES = { # HiProtoCode = 0
                 0: {0x81: "DeliveryFailure",
                     0x09: "HelloCmd",
                     0x89: "HelloResp",
                     0x0e: "HelloReq",
                     0x0d: "ByeMsg",
                     ...
                     0x08: "SetStringsCmd",
                     0x88: "SetStringsResp",
                     0x0f: "GetDevConfigCmd",
                     0x8f:" GetDevConfigResp",
                 ...
                 1: {0xa1: "PleaseWait",
                     0x17: "ClockCmd",
                     0x97: "ClockResp",
                     0x1c: "FileDownloadCmd",
                     0x9c: "FileDownloadResp",
                     0x1d: "FileUploadCmd",
                     0x9d: "FileUploadResp",
                     0x1e: "FileControlCmd",
                     0x9e: "FileControlResp",
                     0x18: "GetProgStatsCmd",
                     0x98: "GetProgStatsResp",
                     0x09: "CollectDataCmd",
                     0x89: "CollectDataResp",
                     0x20: "OneWayTabDefMsg",
                     0x14: "OneWayDataMsg",
                     0x19: "TableControlCmd",
                     0x99: "TableControlResp",
                     0x1a: "GetValuesCmd",
                     0x9a: "GetVaulesResp",
                     0x1b: "SetValuesCmd",
                     0x9b: "SetValuesResp"}                
    ...
    def __init__(self, link, dest_addr=0x001, dest_node=None,
        ...
        if self.link != None:
            LOGGER.info('Get {}\'s attention'.format(self.dest_node))
            self.link.write(b'\xBD\xBD\xBD\xBD\xBD\xBD')

    def log_packet(self, dpkt_hdr, dpkt_msg):
        if dpkt_hdr == {} or dpkt_msg == {}:
            return
        
        if dpkt_hdr['HiProtoCode'] in PakBus.MSGTYPES and   \
                dpkt_msg['MsgType'] in PakBus.MSGTYPES[dpkt_hdr['HiProtoCode']]:
            typestr = PakBus.MSGTYPES[dpkt_hdr['HiProtoCode']][dpkt_msg['MsgType']]
        else:
            typestr = "?"
        
        LOGGER.info("  [ls:{}, em:{}, pri:{}, hi:{}, hc:{}] PhyAddr: {}->{} Node: {}->{}: (type={}-0x{:x}, transID={})".format(
                                        dpkt_hdr['LinkState'],
                                        dpkt_hdr['ExpMoreCode'],
                                        dpkt_hdr['Priority'],
                                        dpkt_hdr['HiProtoCode'],
                                        dpkt_hdr['HopCnt'],
    ...
    def write(self, packet):
        ...
        LOGGER.debug('Packet data: %s' % bytes_to_hex(packet))
        LOGGER.debug('Calculate signature for packet')
        sign = self.compute_signature(packet)
        LOGGER.debug('Calculate signature nullifier to create packet')
        ...
        if LOGGER.getEffectiveLevel() < logging.INFO:            
            self.log_packet(*self.decode_packet(packet[1:-1]))
    ...
    def read(self):
        ...
        LOGGER.info('Read packet:[%d] %s' % (len(packet), bytes_to_hex(packet)))
    ...
    def wait_packet(self, transac_id=None, addressCheck = True):
        ...
        # if no transact id return any message
        if (transac_id == None) or (addressCheck == False) or \
                (msg['TranNbr'] == transac_id):
            return hdr, msg
        else:
            LOGGER.warn("Got message with unexpected transaction id, it will be dropped.")
            return {}, {}

    def pack_header(self, hi_proto, exp_more=0x2,  link_state=None,
                    hops=0x0,  priority= 0x2,):
    ...
    def decode_bin(self, types, buff, length=1):
        ...
        for type_ in types:
            ...
            if type_ == 'ASCIIZ':
                ...
                if nul == -1:
                    value = buff[offset:].decode('utf-8', errors='replace')
                    size = len(buff[offset:])
                else:
                    value = buff[offset:nul].decode('utf-8', errors='replace')                
                    size = nul - offset + 1
                # decodign may change the string length
                # return string without trailing '\0'
                # size = len(value) + 1                    
            elif type_ == 'ASCII':
                # special handling: fixed-length string
                size = length
                value = buff[offset:offset + size].decode('utf-8', errors='replace')
    ...
    def decode_packet(self, data):
        ...
        if len(data) < 8:
            LOGGER.error('Cannot decode packet, not enough bytes for header.')
            return {}, {}
        ...
        if len (msg['raw']) >= 2:
            ...
            LOGGER.debug('HiProtoCode, MsgType = <%x, %x>' %
        ...
        elif hdr['HiProtoCode'] == 1 and msg['MsgType'] == 0x14:
            msg = self.unpack_oneway_data_message(msg)
        else:
            if hdr['HiProtoCode'] in PakBus.MSGTYPES and   \
                    msg['MsgType'] in PakBus.MSGTYPES[hdr['HiProtoCode']]:
                typestr = PakBus.MSGTYPES[hdr['HiProtoCode']][msg['MsgType']]
            else:
                typestr = "?"
            
            LOGGER.error('No implementation for {}-<({:x}, {:x})> packet type'.format(
                         typestr, hdr['HiProtoCode'], msg['MsgType']))
        return hdr, msg
    ...
    def get_hello_cmd(self):
        ...
        hdr = self.pack_header(0x0, 0x1, self.READY)
    ...
    def get_getsetting_fragment(self, settingId, offset= 0):
        '''Create GetsettingsFragment Command packet.'''
        transac_id = self.transaction.next_id()
        hdr = self.pack_header(0x0)
                
        msg = self.encode_bin(['Byte', 'Byte'], [0x11, transac_id])

        return b''.join([hdr, msg]), transac_id
    
    def get_getsettings_cmd(self, beginId= None, endId = None):
        '''Create Getsettings Command packet.'''
        transac_id = self.transaction.next_id()
        hdr = self.pack_header(0x0)
                
        msg = self.encode_bin(['Byte', 'Byte', 'UInt2'], [0x0f, transac_id, self.security_code])
        if beginId != None:
            msg += self.encode_bin(['UInt2'], [beginId])
        if endId != None:
            msg += self.encode_bin(['UInt2'], [endId])
        return b''.join([hdr, msg]), transac_id
    ...
    def unpack_getsettings_response(self, msg):
        ...
        if msg['Outcome'] == 0x01:
            ...
            while offset < len(msg['raw']):
                ...
                # Get value, guess at encoding based on length
                sdecode = {1: 'Byte', 2: 'UInt2', 4: 'UInt4'}
                if SettingLen in sdecode:
                    DecodeValue = self.decode_bin([sdecode[SettingLen]], msg['raw'][offset:offset + SettingLen])[0][0]
                else:
                    DecodeValue = None
    ...
    def unpack_oneway_tabledef(self, msg):
        '''unpack the one way tabledef messagee'''
        offset = 2
        values, size = self.decode_bin(['UInt2'], msg['raw'][offset:])        
        msg['TableNbr'] = values[0]
        offset += size
        
        msg['Tabledef'] = msg['raw'][offset:]
        
        del msg['raw']
        return msg
                

    def unpack_oneway_data_message(self, msg):
        '''unpack the one way message'''
        offset = 2
        values, size = self.decode_bin(['UInt2', 'UInt2', 'UInt4'], 
                                       msg['raw'][offset:])
        msg['TableNbr'], msg['TableDefSig'], msg['RecNbr'] = values
        offset += size
        
        # Decode number of records (16 bits) or ByteOffset (32 Bits)
        values, size = self.decode_bin(['Byte'], msg['raw'][offset:])
        msg['IsOffset'] = values[0] >> 7
                
        if msg['IsOffset'] == 0:
            #number of records, 16 bit value            
            values, size = self.decode_bin(['UInt2'], msg['raw'][offset:])
            msg['NbrOfRecs'] = values[0]            
        else:
            #byte offset, so 32 bit value            
            values, size = self.decode_bin(['UInt4'], msg['raw'][offset:])
            msg['NbrOfRecs'] = values[0] & 0x7fffffff        
                    
        offset =+ size        
      
        # fragmented messages only have a timestamp on the first frag.
        #       
        values, size = self.decode_bin(['NSec'], msg['raw'][offset:])
        msg['TimeOfRec'] = nsec_to_time(values[0])
        offset += size
        
        msg['RecFrag'] = msg['raw'][offset:]
        return msg
    ...
    def parse_oneway_data(self, data, tabledef):
                
        frag = {}
        # Loop over all records
        
        if data['MsgType'] != 0x14:
            raise Exception("Wrong message type")
        
        # Provide table name
        t_frag = tabledef[data['TableNbr'] - 1]
                
        if (t_frag['Signature'] != data['TableDefSig']):
            raise Exception("Table signature changed.")
        ...
        for n in range(data['NbrOfRecs']):
            record = {}

            # Calculate current record number
            record['RecNbr'] = data['RecNbr'] + n        
            
            record['TimeOfRec'] = data['TimeOfRec']
            #values, size = self.decode_bin(['NSec'], data['RecFrag'][offset:])
            #record['TimeOfRec'] = nsec_to_time(values[0], True)                                                  
            #offset += size
            # Loop over all field indices
            record['Fields'] = {}
            ...
            for field in fields:
                ...
                fieldtype = t_frag['Fields'][field - 1]['FieldType']
                dimension = t_frag['Fields'][field - 1]['Dimension']
                print("decode {} dim = {}".format(fieldname, dimension))
                ...
                if fieldtype == 'ASCII':
                    ...
                    values, size = \
                        self.decode_bin(dimension * [fieldtype],
                                        data['RecFrag'][offset:])
                    record['Fields'][fieldname] = values[0]
                print("size was {}".format(size))
                offset += size
            frag['Records'].append(record)
        print("final offset = {}".format(offset))
        print("length = {}".format(len(data['RecFrag'])))
        return frag 
