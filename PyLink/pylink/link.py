...
class TCPServerLink(Link):
    '''TCPLink class allows TCP/IP server protocol communication with File-like
    API.'''
    def __init__(self, host, port, timeout=1):
        self.timeout = timeout
        self.host = host #socket.gethostbyname(host)
        self.port = port
        self._socket = None
    ...
    def url(self):
        '''Make a connection url from `host` and `port`.'''
        return 'tcpsvr:%s:%d' % self.address
